...
async function testExploreSettings(page) {
	const errors = [],
		failed = [],
		checks = [];
	...
	await wait(() => window.__blackHoleStats?.frame > 5);
	...
	await page.waitForFunction(
		(t) => window.__blackHoleStats.shaderTime > t + 0.6,
		beforeTime,
	);
	const initial = await stats();
	const initialPercent = await slider.inputValue();
	await page.evaluate(() => {
		window.__exploreCanvas = document.querySelector(
	...
	await page.waitForFunction(
		() => document.querySelector("#explore-resolution").value === "26",
	);
	...
	assert(
		await page.evaluate(
			() =>
				...
				document.querySelector("[data-black-hole-background] canvas"),
		),
