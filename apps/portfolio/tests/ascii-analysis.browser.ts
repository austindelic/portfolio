// Run via Playwright CLI: run-code --filename=apps/portfolio/tests/ascii-analysis.browser.ts
export default async function testAsciiAnalysis(
  ...
  return await page.evaluate(async () => {
    const {
      ...
      ascii,
      analysisWgsl,
    } = await import("/tests/shader-fixtures.ts");
    const gl = document.createElement("canvas").getContext("webgl2")!;
    if (!gl) throw Error("WebGL2 unavailable");
    const compile = (type: number, src: string) => {
      const sh = gl.createShader(type);
      if (!sh) throw Error("Shader allocation failed");
      gl.shaderSource(sh, src);
      gl.compileShader(sh);
      if (!gl.getShaderParameter(sh, gl.COMPILE_STATUS))
        throw Error(gl.getShaderInfoLog(sh) || "Shader compilation failed");
      return sh;
    };
...
${source}`,
    ...
    const texture = (w: number, h: number, data: Uint8Array | null) => {
    ...
    const cases: {
      ...
      history: boolean;
      size: [number, number];
      result: { color: number[]; state: number[] };
    }[] = [];
    const run = (
      samples: number[],
      metrics: number[][],
      ...
      const inputs = [
        texture(
          ...
          4,
          new Uint8Array(
            samples.flatMap((v: number) => [v * 255, v * 255, v * 255, 255]),
          ),
        ),
    ...
    const assert = (ok: boolean, name: string | undefined) => {
    ...
    const aspect = run(
      samples((x: number, y: number) => 0.5 + ((y - 1.5) * 2 - (x - 1.5)) / 12),
      metrics,
      0,
      0,
      false,
    ...
    const restrained = run(
      samples((x: number) => (x < 2 ? 127 : 129) / 255),
      metrics,
      0,
      0,
      false,
    ...
    assert(
      restrained.state[0] === 1 && restrained.state[1] > 0,
      "weak contour retains letters while boundary confidence stays independent",
    );
    const cancelled = run(
      samples((x: number, y: number) => (x === 1 && y === 1 ? 1 : 0.5)),
      metrics,
    );
    assert(
      cancelled.state[0] === 1,
      "cancelled Sobel neighbourhood rejects directional candidates",
    );
    const flat = run(
      samples(() => 0.5),
      metrics,
    ...
    const peak = run(
      samples((x: number, y: number) => (x === 0 && y === 0 ? 1 : 0)),
      metrics,
    );
    assert(
    ...
    const priority = run(
      samples((x, y: number) => (y > 1 ? 0.9 : 0.1)),
      [
        [0, 0],
        [0.5, 0],
        [0.1, 1],
 ...
 out vec4 result;void main(){mainImage(result,gl_FragCoord.xy);}`,
    ...
    if (!gl.getProgramParameter(composite, gl.LINK_STATUS))
      throw Error(gl.getProgramInfoLog(composite) || "Program link failed");
    ...
    const boundaryInputs = [
      texture(
        ...
        4,
        new Uint8Array(
          samples((x, y: number) => (y > 1 ? 1 : 0)).flatMap((v) => [
            v * 255,
            v * 255,
            v * 255,
            255,
    ...
    for (const c of cases) {
      ...
      const bind = device.createBindGroup({
        ...
        entries: [input, oldState, metrics, oldColor, color, state]
          .map<GPUBindGroupEntry>((t, binding) => ({
            binding,
      ...
      assert(
        c.result.color.every(
          (v: number, i: number) => Math.abs(v - pixels[i]) <= 1,
        ) &&
          c.result.state.every(
            (v: number, i: number) => Math.abs(v - pixels[256 + i]) <= 1,
          ),
