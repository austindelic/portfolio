...
async (page) => {
	...
	await page.addInitScript(() => {
		...
		for (const [name, set] of [
			...
			const create = WebGL2RenderingContext.prototype["create" + name],
				remove = WebGL2RenderingContext.prototype["delete" + name];
			WebGL2RenderingContext.prototype["create" + name] = function (...args) {
				const value = create.apply(this, args);
				if (value) set.add(value);
				return value;
			};
			WebGL2RenderingContext.prototype["delete" + name] = function (value) {
				set.delete(value);
	...
	const snapshot = () =>
		page.evaluate(() => ({
			...
			clock: document.querySelector("#perth-time")?.textContent,
		}));
	...
	for (let visit = 0; visit < 2; visit++) {
		...
		await page.waitForFunction(() => window.__blackHoleStats?.frame > 2);
