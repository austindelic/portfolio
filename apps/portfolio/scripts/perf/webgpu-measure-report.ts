...
const report: Awaited<
  ReturnType<typeof import("./webgpu-measure.ts").default>
...
const trials = report.results.map((r) => {
  const spacing = r.frames
    .slice(1)
    .map(
      (f: { timestamp: number }, i: number) =>
        f.timestamp - r.frames[i].timestamp,
    );
  return {
    viewport: r.viewport,
    ...
    slowFrameFraction:
      spacing.filter((t: number) => t > 25).length / spacing.length,
...
for (const width of [1440, 2560, 390])
  for (const backend of ["webgl2", "webgpu"]) {
    const group = trials.filter(
      (r: { viewport: { width: number }; backend: string }) =>
        r.viewport.width === width && r.backend === backend,
    );
