...
async (page) => {
	...
	phone.on("requestfailed", (r) => failedRequests.push(r.url()));
	phone.on("request", (r) => {
		if (
			/BlackHole(?:Runtime|Core|WebGl|WebGpu|CellShaders|Gl|Shader|Controls)/.test(
				r.url(),
			)
		)
	...
	const checkStatic = async () => {
		...
		const state = await phone.evaluate(() => ({
			...
			live: document.querySelector("[data-black-hole-background]").dataset
				.bhLive,
			canvasDisplay: getComputedStyle(
				document.querySelector(".bh-background-live"),
			).display,
			imageVisibility: getComputedStyle(
				document.querySelector(".bh-background-still"),
			).visibility,
		}));
		if (
			state.contexts ||
	...
	const navigate = async (action, url) => {
		const before = await phone.evaluate(() => window.__pageLoads);
		await action();
		await phone.waitForURL(url);
		await phone.waitForFunction(previous => window.__pageLoads > previous, before);
		await checkStatic();
	};
	try {
		...
		await phone.evaluate(() => {
			window.__originalBackground = document.querySelector(
				"[data-black-hole-background]",
			);
			window.scrollTo(0, 650);
		});
		await checkStatic();
		await navigate(() => phone.locator('a[href="/blog"]').first().click(), "**/blog");
		if (
			!(await phone.evaluate(
				() =>
					window.__originalBackground ===
					document.querySelector("[data-black-hole-background]"),
			))
		)
			throw Error("Background was not persisted");
		await navigate(() => phone.locator('a[href="/socials"]').first().click(), "**/socials");
		await navigate(() => phone.goBack(), "**/blog");
		await navigate(() => phone.goForward(), "**/socials");
		await navigate(() => phone.locator('a[href="/"]').first().click(), `${origin}/`);
