interface State {
  atlasConfig: { textSize: number; fontFamily: string };
  controls: Record<string, unknown>;
  runtimeSnapshot: { frame: number };
  requests?: number;
  resets?: number;
...
interface Runtime {
  updateRoute(route: string, reset?: boolean): void;
  updateSettings(settings: { asciiMix: number }): void;
...
function fixture(fontReady = true) {
  ...
  const mount = new Function(
    ...
    (canvas: object, state: State, options: Options) => {
    ...
    { fonts },
    String,
    (controls: Record<string, unknown>) => ({ ...controls }),
  );
  const state: State = {
