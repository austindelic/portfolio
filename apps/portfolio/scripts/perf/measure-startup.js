// Startup repeat only: no steady-state metrics should be used from this short run.
...
async (page) => {
	...
	await page.addInitScript(() => {
		...
		for (const kind of [
			...
			for (const action of ["create", "delete"]) {
				const name = action + kind,
					original = WebGL2RenderingContext.prototype[name];
				WebGL2RenderingContext.prototype[name] = function (...args) {
		...
		for (const name of [
			...
			WebGL2RenderingContext.prototype[name] = function (...args) {
				...
				if (name.startsWith("uniform"))
					probe.uniforms[names.get(args[0])] = args
						.slice(1)
						.map((v) => (ArrayBuffer.isView(v) ? Array.from(v) : v));
				return original.apply(this, args);
			};
		}
		...
		HTMLCanvasElement.prototype.getContext = function (...args) {
			...
			if (args[0] === "webgl2" && gl) probe.gl = gl;
			return gl;
		};
					...
					await page.evaluate(
						({ viewport, run, port }) => {
							...
							return {
								...
								droppedFrames: wall.filter((v) => v > 25).length,
								drawsPerFrame:
