...
// Run from repo root: bun apps/portfolio/scripts/perf/summarize.mjs BASELINE_DIST FINAL_DIST LOG_DIR
...
async function initialAssets(directory) {
...
const baseline = await result("baseline-measure"),
	final = await result("final-measure");
const summaries = [];
for (const width of [1440, 2560, 390]) {
	const select = (data) =>
		data.results.filter((r) => r.viewport.width === width);
