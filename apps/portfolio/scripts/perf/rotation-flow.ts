// Run intro-flow.ts first in the same Playwright session to install its controlled clock.
export default async (page: import("@playwright/test").Page) => {
  const results = [];
  const errors: string[] = [];
  page.on("pageerror", (e: { message: string }) => errors.push(e.message));
  page.on("requestfailed", (r: { url: () => string }) => errors.push(r.url()));
  ...
  for (const size of [
    ...
    for (const [path, period] of views) {
      ...
      const closure = Math.hypot(
        ...final.map((v: number, i: number) => v - first[i]),
      );
      ...
      const aimClosure = Math.hypot(
        ...finalAim.map((v: number, i: number) => v - firstAim[i]),
      );
