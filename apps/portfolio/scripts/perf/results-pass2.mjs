...
for (const name of logs) {
	...
	const data = JSON.parse(log.split("### Result\n")[1].split("\n### Ran")[0]);
	await writeFile(
	...
	if (data.results?.[0]?.port) {
		...
		for (const width of [1440, 2560, 390])
			for (const port of [4400, 4401]) {
				...
				const row = { width, port, samples: samples.length };
				for (const key of [
					"median",
					"p95",
					"cpuMedian",
					...
					if (samples.every((r) => typeof r[key] === "number"))
						row[key] = median(samples.map((r) => r[key]));
				summary.push(row);
			}
		await writeFile(
...
if (process.env.CAPTURE_BEFORE && process.env.CAPTURE_AFTER) {
	...
	for (const file of (await readdir(".playwright-cli")).filter(
		(f) => f.startsWith(before + "-") && !(after !== before && f.startsWith(after + "-")) && f.endsWith(".png") && (!suffix || /-\d+\.png$/.test(f)),
	)) {
