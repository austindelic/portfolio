...
export default async (page: import("@playwright/test").Page) => {
  ...
  await page.waitForFunction(
    () => (window.__blackHoleAnimationSnapshot?.shaderTime ?? 0) > 10,
  );
  await page.evaluate(() => {
  ...
  const navigate = async (path: string, delay = 8200) => {
    ...
    await page.waitForURL((url: { pathname: string }) => url.pathname === path);
      ...
      assert(
        Math.hypot(
          ...current.cameraPosition.map(
            (v: number, i: number) => v - anchors[path][i] * scale,
          ),
        ) < 0.5,
