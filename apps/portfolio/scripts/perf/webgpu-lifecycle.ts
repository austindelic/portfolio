export default async (page: import("@playwright/test").Page) => {
  ...
  for (const scenario of [
    ...
    await context.addInitScript((scenario: string) => {
      window.__gpuAudit = {
        devices: 0,
        destroyed: 0,
        textures: 0,
      ...
      if (scenario === "both-unavailable") {
        ...
        HTMLCanvasElement.prototype.getContext = function (
          ...
          return type === "webgl2"
            ? null
            : (Reflect.apply(original, this, [type, ...args]) as
                | RenderingContext
                | GPUCanvasContext
                | null);
    ...
    if (scenario === "both-unavailable")
      await p.waitForFunction(() =>
        document
          .querySelector<HTMLElement>("[data-black-hole-error]")!
      ...
      await p.waitForFunction(
        () => (window.__blackHoleStats?.frame ?? 0) > 2,
        null,
        {
          timeout: 60000,
        },
