...
function fixture(mobile = true) {
	...
	const host = {
		dataset: { settings: "{}" },
		isConnected: true,
		querySelector: (selector) => (selector === "canvas" ? canvas : error),
	};
	...
	const sessions = [],
		loads = [];
	const module = {
		createRuntimeState: (props) => props,
		runtimeOptions: (_props, onError) => ({ onError }),
		mountBlackHoleRuntime(_canvas, state, options) {
			const session = {
				state,
				options,
				disposals: 0,
