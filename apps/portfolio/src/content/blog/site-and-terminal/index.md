...
The underlying physics shader comes from the existing code referenced in [Buffer A](https://github.com/austindelic/austindelic/blob/main/packages/black-hole/shaders/buffer-a.glsl), including [baopinshui's NPGS](https://github.com/baopinshui/NPGS). The bloom and image passes are adapted from sonicether's “Gargantua With HDR Bloom”. Those source credits remain in the shader files.
...
```sh
git clone https://github.com/austindelic/austindelic.git
cd austindelic
...
The apps live in `apps/portfolio` and `apps/tui`. The shared shader sources are in `packages/black-hole`.
