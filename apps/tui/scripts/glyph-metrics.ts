export default async (page: import("@playwright/test").Page) => {
  ...
  const result = await page.evaluate(async () => {
    ...
    } = await import("/src/components/BlackHoleCore.ts");
    ...
    const coverage = glyphs.map((glyph) => {
      const r = createGlyphAtlasRaster({
        ...config,
        glyphs: glyph,
        glyphCount: 1,
      });
      const pixels = r.canvas
        .getContext("2d")!
        .getImageData(0, 0, r.canvas.width, r.canvas.height).data;
      let sum = 0;
      for (let i = 3; i < pixels.length; i += 4) sum += pixels[i];
      return sum / (255 * r.canvas.width * r.canvas.height);
    });
    return {
      glyphs: rankGlyphs(coverage)
        .map((x: { index: number }) => glyphs[x.index])
        .join(""),
      metrics: Array.from(
