...
assert.match(id || '', /^\d+$/);
const base = 'https://api.github.com/repos/austindelic/austindelic/actions';
async function get(url) {
  const response = await fetch(url, { headers: { Authorization: `Bearer ${process.env.GITHUB_TOKEN}`, Accept: 'application/vnd.github+json' } });
  assert.ok(response.ok, `Cannot validate original run: ${response.status}`);
  return response.json();
}
const run = await get(`${base}/runs/${id}`);
assert.equal(run.head_branch, 'main');
assert.equal(run.head_repository.full_name, 'austindelic/austindelic');
assert.equal(run.path, '.github/workflows/release-tui.yml');
assert.ok(['push', 'workflow_dispatch'].includes(run.event));
assert.equal(run.status, 'completed', 'Original run must finish before recovery');
const { jobs } = await get(`${base}/runs/${id}/jobs?filter=latest&per_page=100`);
assert.ok(jobs.some(job => job.name === 'tested-bundle' && job.conclusion === 'success'), 'Both package sets must have passed the full test matrix');
