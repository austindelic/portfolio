...
import * as launcher from "../../npm/bin/cli.ts";
...
function fixture(t: import("node:test").TestContext) {
  ...
  const bundle: ReleaseBundle = {
    ...
    version: "1.0.0",
    tag: "tui-v1.0.0",
    notes: "Saved release notes",
    packages: [],
  };
  const budgets: SizeBudgets = {};
  for (const [registry, { prefix, url }] of Object.entries(
    publisher.registries,
  )) {
    mkdirSync(path.join(cwd, "packed", registry), { recursive: true });
    for (const name of [
      "austindelic",
      ...launcher.targets.map((t) => `austindelic-${t}`),
    ]) {
  ...
  const save = () =>
    writeFileSync(
      path.join(cwd, "packed/release.json"),
      JSON.stringify(bundle),
    );
  save();
  const calls: string[] = [];
  const publish = async (folder: string) => {
    assert.equal(folder, path.join(cwd, "packed"));
    assert.equal(
      git("--git-dir", remote, "rev-parse", `${bundle.tag}^{commit}`),
      sha,
      "Remote tag must exist before publishing",
    );
    calls.push("packages");
  };
  const createRelease = async (actual: ReleaseBundle) => {
    assert.deepEqual(actual, bundle);
    calls.push("release");
  };
  const run = (overrides = {}) =>
    publishSnapshot({ cwd, publish, createRelease, ...overrides });
  const plan = () =>
    semanticRelease(
      {
        branches: ["main"],
...
test("publish the planned snapshot after main advances, then plan from the published tag", async (t) => {
  ...
  assert.ok(initial);
  assert.equal(initial.nextRelease.version, f.bundle.version);
...
test("an existing annotated tag at the tested commit can resume publication", async (t) => {
  ...
  await f.run();
  assert.equal(
    f.git("--git-dir", f.remote, "rev-parse", f.bundle.tag),
    original,
  );
  assert.deepEqual(f.calls, ["packages", "release"]);
});
...
test("a rejected tag push stops package publication", async (t) => {
  const f = fixture(t);
  writeFileSync(
    path.join(f.remote, "hooks/pre-receive"),
    "#!/bin/sh\nexit 1\n",
    { mode: 0o755 },
  );
...
test("invalid artifacts, budgets, source and verification-only plans cannot create tags", async (t) => {
  for (const kind of ["archive", "budget", "source", "verification"]) {
    ...
    if (kind === "archive")
      appendFileSync(
        path.join(f.cwd, "packed/npm/austindelic.tgz"),
        "tampered",
      );
    ...
    await assert.rejects(
      f.run(),
      /changed after testing|exceeds accepted size|Checkout must match|Verification-only/,
    );
...
test("partial publication keeps its tag and resumes identical packages before creating a release", async (t) => {
  ...
  assert.deepEqual(
    f.calls,
    [],
    "No GitHub Release for an incomplete publication",
  );
  ...
  assert.equal(
    sent.length,
    12,
    "Already published archives are not republished",
  );
