...
export async function publishSnapshot({ cwd = process.cwd(), directory = 'packed',
  publish = publisher.publishAll, createRelease }) {
  const git = (...args) => execFileSync('git', args, { cwd, encoding: 'utf8', stdio: ['ignore', 'pipe', 'pipe'] }).trim();
  const folder = path.resolve(cwd, directory);
  const bundle = JSON.parse(readFileSync(path.join(folder, 'release.json')));
  ...
  publisher.validateBudgets(bundle, JSON.parse(readFileSync(path.join(cwd, 'apps/tui/scripts/size-budgets.json'))));
  ...
  git('fetch', '--no-tags', '--prune', 'origin',
    '+refs/heads/main:refs/remotes/origin/main', '+refs/tags/*:refs/release-tags/*');
  const ancestry = spawnSync('git', ['merge-base', '--is-ancestor', bundle.sha, 'refs/remotes/origin/main'], { cwd, encoding: 'utf8' });
  if (ancestry.error) throw ancestry.error;
  assert.ok(ancestry.status === 0 || ancestry.status === 1, `Cannot check main ancestry: ${ancestry.stderr}`);
  ...
  const refs = git('for-each-ref', '--format=%(refname)', 'refs/release-tags/').split('\n');
  ...
  for (const ref of refs) {
    ...
    assert.ok(index < 0 || parts[index] < current[index],
  ...
  if (refs.includes(tagRef)) {
    assert.equal(git('rev-parse', `${tagRef}^{commit}`), bundle.sha,
